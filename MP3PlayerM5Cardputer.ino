...
void play(const char* fname, const char* fname2) {
  if (id3 != nullptr) { stop(); }
  M5Cardputer.Display.setCursor(0, 8);
  file.open(fname);
  id3 = new AudioFileSourceID3(&file);
  //  id3->RegisterMetadataCB(MDCallback, (void*)"ID3TAG");
  M5Cardputer.Display.drawString("                                        ", 1, 8);
  M5Cardputer.Display.drawString(fname2, 1, 8);
  //  M5Cardputer.Display.printf(fname2);
  mp3.begin(id3, &out);
}
...
void resume(const char* fname, const char* fname2) {
  if (id3 != nullptr) { stop(); }
  M5Cardputer.Display.setCursor(0, 8);
  file.open(fname);
  id3 = new AudioFileSourceID3(&file);
...
void setup(void) {
  ...
  filetoplay2 = files2[fileindex];
  filetoplay.toCharArray(f, 256);
  filetoplay2.toCharArray(g, 256);
  play(f, g);
}
